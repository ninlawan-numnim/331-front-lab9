...
const filteredItems = computed(() => {
  return auctionItems.value.filter(item => {
    ...
    return matchDescription && matchType
  })
})
...
const getBidCount = (item: AuctionItem) => {
      ...
      >
        ...
        <div class="mt-4 pt-4 border-t">
          <p class="text-sm text-gray-700">
            <strong>Bids:</strong> {{ getBidCount(item) }}
          </p>
          <p class="text-sm text-gray-700">
            <strong>Highest Bid:</strong> ${{ getHighestBid(item).toFixed(2) }}
          </p>
          <p v-if="item.successfulBid" class="text-green-600 font-semibold mt-2">
